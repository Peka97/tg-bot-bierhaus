...
def get_full_date():
	return timezone("Europe/Saratov").localize(datetime.now()).date().strftime("%d.%m.%Y")
...
def get_current_week_number():
	now = timezone("Europe/Saratov").localize(datetime.now())
...
def list_exist():
	...
	sheets = service.spreadsheets().get(spreadsheetId=SPREADSHEET_ID_MAIN).execute().get('sheets')
	for sheet in sheets:
...
def get_shop_row(shop_name: str, time: str):
	...
	if time == 'day':
		addresses = [address[:-10] for address in values if ' (Дневная)' in address]
	elif time == 'day_only':
...
def update_meters_table_fields(user_id: int):
	try:
		...
		user_data = get_user_info(user_id)
		print(f"Не удалось найти магазин {get_shop_name(user_data['shop'], user_data['time'])}")
...
def create_meters_list():
	try:
		...
		current_month = timezone("Europe/Saratov").localize(datetime.now()).date().strftime("%m.%Y")
		date = timezone("Europe/Saratov").localize(datetime.now()).date().strftime("%B %Y")
		title = f'{current_month} Счётчики'
...
def create_consumables_list():
	try:
		pattern_sheet_id = get_sheet_id('Consumables Pattern', SPREADSHEET_ID_MAIN)
		current_month = timezone("Europe/Saratov").localize(datetime.now()).date().strftime("%m.%Y")
		date = timezone("Europe/Saratov").localize(datetime.now()).date().strftime("%d.%m.%Y")
...
def create_finance_user_list():
	try:
		pattern_sheet_id = get_sheet_id('Finance User Pattern', SPREADSHEET_ID_FINANCES)
...
def update_finance_user_list(user_id: int, comment: str, cash: dict):
	...
	current_month = timezone("Europe/Saratov").localize(datetime.now()).date().strftime("%m.%Y")
	current_time = timezone("Europe/Saratov").localize(datetime.now()).strftime("%d.%m.%y %H:%M")
	...
	if 'loss' in cash.keys():
		...
		values.append([comment, ' ', cash['collection'], 'enter', current_time])
...
def create_finance_staff_list():
	pattern_sheet_id = get_sheet_id('Finance Staff Pattern', SPREADSHEET_ID_FINANCES)
	current_month = timezone("Europe/Saratov").localize(datetime.now()).date().strftime("%m.%Y")
